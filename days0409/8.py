...
#     with open("./img2/%d.jpg" % res, "wb")as file:
...
def main():
    ...
    # pool = Pool(31)
    # for r in range(31):
    #     pool.apply_async(write,args=(r,list[r],))
    # pool.close()
    # pool.join()
    ...
    for i in range(32):
        t = Thread(target=write,args=(i,list[i],))
        t.start()
        img.append(t)
...
def write(*args):
    res = args[0]
    response = requests.get(args[1])
    with open("./img3/%d.jpg" % res, "wb")as file:
        file.write(response.content)
        print("下载完成")

    # p = Process(target=write,args=(list,))
    # p.start()
